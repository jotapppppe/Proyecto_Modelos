...
datos$`NPI_group` <- factor(datos$`NPI_group`,levels = c("excellent", "good", "moderate 1", "moderate 2", "poor"))
...
datos$`Cancer_grade` <- factor(ifelse(datos$`Cancer_grade` %in% c("Grade 1", "Grade 2"), 
                                     "Grade 1-2", "Grade 3")) # se agrupan pues hay pocas obs en 1 y 2
datos$`NPI_group` <- factor(ifelse(datos$`NPI_group` %in% c("excellent", "good"), 
                                  "good/excellent",
                                  ifelse(datos$`NPI_group` %in% c("moderate 1", "moderate 2"),
                                         "moderate",
                                         "poor")))
...
Modelo
```{r}
modelo.cox <- coxph(Surv(`10yr_survival_(months)`, `Survival_status_at_10yr_follow-up_(months)`) ~ Age +
                      `Presenting_Symptom` +
                      `Cancer_Size` +
                      ...
                      `Surgical_procedure` +
                      `NPI_10_yr_survival_(%)` +
                      `Adjuvant_online_10yr_survival_(%)`, data = datos)
cox.zph(modelo.cox)
```
